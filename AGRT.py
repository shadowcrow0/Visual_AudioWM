...
# --- scipy.stats.mvn.mvnun was removed in SciPy 1.12; fall back if absent ---
try:
    ...
    def _mvn_cdf(point, mean, cov):
        """P(X <= point) for a multivariate normal, tolerating +/-inf coordinates."""
        point = np.asarray(point, dtype=float)
        if np.any(point == -np.inf):
            return 0.0
        keep = np.isfinite(point)
        if not keep.any():
            return 1.0
        idx = np.flatnonzero(keep)
        m = np.asarray(mean, dtype=float)[idx]
        c = np.asarray(cov, dtype=float)[np.ix_(idx, idx)]
        if idx.size == 1:
            return float(stats.norm.cdf(point[idx][0], loc=m[0], scale=np.sqrt(c[0, 0])))
        return float(_mvnorm(mean=m, cov=c, allow_singular=True).cdf(point[idx]))

    def mvnun(lower, upper, means, covar, maxpts=None, abseps=1e-6, releps=1e-6):
        """Rectangle probability P(lower < X < upper) by inclusion-exclusion.

        Returns (value, inform) so existing `mvnun(...)[0]` call sites keep working.
        """
        ...
        for bits in itertools.product((0, 1), repeat=d):
            corner = np.where(np.asarray(bits, dtype=bool), upper, lower)
            total += (-1) ** (d - sum(bits)) * _mvn_cdf(corner, means, covar)
        return (float(min(max(total, 0.0), 1.0)), 0)
