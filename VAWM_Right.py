...
def load_rules(rule_path):
    ...
    with open(rule_path, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            # Handle both 'ResponseBox' and 'ResposeBox' column names
            response_col = 'ResposeBox' if 'ResposeBox' in row else 'ResponseBox'
            response = int(row[response_col].strip())
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    os.chdir(_thisDir)
    deviceManager.addDevice(
            deviceClass='psychopy.hardware.speaker.SpeakerDevice', deviceName='Speakers (Realtek(R) Audio)', index=8
        )
    ...
    sd.default.device = 'Speakers (Realtek(R) Audio)'
    Fix = visual.TextStim(win=win, name='Fix',
        text='+',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.2, wrapWidth=None, ori=0.0, 
        color='#FFFFFF', colorSpace='hex', opacity=1.0, 
        languageStyle='LTR',
        depth=-1.0);
    stim1 = visual.Rect(
    ...
    for thisPractice in practice:
        ...
        for thisInner in inner:
            ...
            thisExp.addData('Probe', thisRepN)
            ...
            inner.addData('Response',ResponseBox.keys)
            if ResponseBox.keys != None:  # we had a response
                inner.addData('RT', ResponseBox.rt)
            ...
            # Cedrus key: 3=no, 4=yes (matches rule.csv integer format)
    ...
    for thisBlock1 in block1:
        ...
        for thisSFT1 in SFT1:
            ...
            if ResponseBox.keys in ['', [], None]:  # No response was made
                ResponseBox.keys = None
            SFT1.addData('Response',ResponseBox.keys)
            if ResponseBox.keys != None:  # we had a response
                SFT1.addData('RT', ResponseBox.rt)
            #    SFT1.addData('ResponseBox.duration', ResponseBox.duration)
            # Calculate Acc based on condition and response
            Response = ResponseBox.keys
            if condition in [0, 1]:  # AA: target color + target audio → Response 4 is correct
                Acc = 1 if Response == 4 else 0
            else:  # Other conditions → Response 3 is correct
                Acc = 1 if Response == 3 else 0
            thisExp.addData('Acc', Acc)
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            if probe.maxDurationReached:
                routineTimer.addTime(-probe.maxDuration)
    ...
    for thisBlock2 in block2:
        ...
        for thisSFT_2 in SFT_2:
            ...
            if ResponseBox.keys in ['', [], None]:  # No response was made
                ResponseBox.keys = None
            SFT_2.addData('Response',ResponseBox.keys)
            if ResponseBox.keys != None:  # we had a response
                SFT_2.addData('RT', ResponseBox.rt)
    ...
    for thisBlock3 in block3:
        ...
        for thisSFT_3 in SFT_3:
            ...
            if ResponseBox.keys in ['', [], None]:  # No response was made
                ResponseBox.keys = None
            SFT_3.addData('Response',ResponseBox.keys)
            if ResponseBox.keys != None:  # we had a response
                SFT_3.addData('RT', ResponseBox.rt)
    ...
    for thisBlock4 in block4:
        ...
        for thisSFT_4 in SFT_4:
            ...
            if ResponseBox.keys in ['', [], None]:  # No response was made
                ResponseBox.keys = None
            SFT_4.addData('Response',ResponseBox.keys)
            if ResponseBox.keys != None:  # we had a response
                SFT_4.addData('RT', ResponseBox.rt)
    ...
    for thisBlock5 in block5:
        ...
        for thisSFT_5 in SFT_5:
            ...
            if ResponseBox.keys in ['', [], None]:  # No response was made
                ResponseBox.keys = None
            SFT_5.addData('Response',ResponseBox.keys)
            if ResponseBox.keys != None:  # we had a response
                SFT_5.addData('RT', ResponseBox.rt)
