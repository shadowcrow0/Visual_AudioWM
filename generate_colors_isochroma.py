...
DH_CH_IN = 15.0     # CH 色相角內緣(度): 15       -> ΔE ≈10(下限)
DH_CH    = 30.0     # CH 色相角外緣(度): 15 ~ 30   -> ΔE 10~20
DH_CL_IN = 38.0     # CL 色相角內緣(度): 38       -> ΔE ≈25(下限)
DH_CL    = 60.0     # CL 色相角外緣(度): 38 ~ 60 的環帶 -> ΔE 25~39
N_TRIALS = 180
SEED     = 11
...
def make_triplet():
    ...
    dh_ch = rng.uniform(DH_CH_IN, DH_CH)  * rng.choice([-1, 1])
    dh_cl = rng.uniform(DH_CL_IN, DH_CL) * rng.choice([-1, 1])
