...
### 6.1 建議:走 SNR 路線(自然 token + 噪音)

**證據強度:中等偏強。**四條理由,依強度排序:
...
| WM 負荷 | 機制不適用於單一音節 ✅(推論);但不能用「品質好」辯護([[francis2009]]) | 無此顧慮 ✅ |
...
2. **論文裡替 SNR 路線辯護時,用 [[silbert2012]] 的方法論論證,不要用生態效度論證。**兩者性質不同([[hamilton2020]] 是生態效度式的,[[silbert2012]] 是模型假設式的),後者對 GRT 審稿人有力得多。
3. **不要主張「合成不可行」。**[[logan1989]] 擋得住這個主張。正確的寫法是「兩條路線都可行,我們基於 GRT 的維度解釋選了自然刺激」。
4. **預期自然刺激的邊界變異較大**([[mcmurray2008]]):適應程序的先驗要放寬,試次預算要留餘裕。
5. **limitation 段必寫**:遮蔽噪音會改變線索權重([[winn2013]]),因此適應階段估到的閾值不能解讀為「純粹的 VOT 敏感度」。
6. **三件必須補做的查證**(見 §7)。
